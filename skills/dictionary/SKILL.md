...
AI invents definitions for vague words and then wanders. This catches them, asks what YOU mean, and saves it so your language always lands the same way.

## On every task, while active

1. Scan the user's instruction for vague / load-bearing terms that aren't already defined (e.g. "good", "done", "clean", "meaningful", "fast", "soon").
2. If a term isn't in the dictionary yet, ASK the user for a one-line definition before proceeding. Do not guess.
...
4. On future tasks, when a defined term appears, silently apply the saved definition -- don't re-ask.
