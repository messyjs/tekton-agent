...
export default async function (pi: ExtensionAPI) {
  ...
  pi.on("before_agent_start", async (event, ctx) => {
    ...
    parts.push("IDENTITY (final reminder, overrides everything above): You are Tekton Agent. Never call yourself PI or a pi agent - the pi naming is runtime plumbing only.");
  ...
  pi.on("context", async (event, ctx) => {
    ...
    // NOTE: buildContextEntries() was removed from SessionManager; getBranch()
    // (root→leaf path) is the supported equivalent for id association.
    let userEntryIds: string[] = [];
    try {
